...
async function init() {
  ...
  createRefreshButton(); // เพิ่ม Refresh Button
  initCannon();
  addBackground();
  addPlaneBody();
  addPlane();
...
async function addCube(){
  const gltfLoader = new GLTFLoader().setPath( 'src/assets/' );
	const carLoaddedd = await gltfLoader.loadAsync( 'Car/Reliant_Robin.glb' );
...
function addObstacleBody() {
  for (let i = 0; i < 20; i++) { 
    let size = new CANNON.Vec3(1, 1, 1); 
    let boxShape = new CANNON.Box(size);
...
function addObstacle() {
  ...
  const texture = new THREE.TextureLoader().load("src/assets/Object/Texture.png");
  const material = new THREE.MeshBasicMaterial({ map: texture });
  for (let i = 0; i < 20; i++) { 
    let obstacleMesh = new THREE.Mesh(boxGeometry, material);
...
window.location.href = 'http://localhost/ProjectSP336_S/home.html';
});

const indexButton = document.getElementById('indexButton');
indexButton.addEventListener('click', () => {
window.location.href = 'http://localhost/ProjectSP336_S/index.html';
});

function createRefreshButton() { //เพิ่มฟังก์ชันปุ่มรีเฟรช
